...
export default class BikeMap extends React.Component {
  ...
  renderMap = res => {
    ...
    var serviceGroupPointList = [];
    if (serviceList != null && serviceList.length > 0) {
      serviceList.forEach(item => {
        var serviceLst = item.data;
        var servicePointLst = [];
        serviceLst.forEach(itemData => {
          let point = new window.BMap.Point(itemData.lon, itemData.lat);
          servicePointLst.push(point);
        });
        serviceGroupPointList.push(servicePointLst);
      });
      //   serviceList.forEach(item => {
    ...
    if (serviceGroupPointList != null && serviceGroupPointList.length > 0) {
      for (var index = 0; index < serviceGroupPointList.length; index++) {
        var itemData = serviceGroupPointList[index];
        if (itemData != null && itemData.length > 0) {
          var polyServiceLine = new window.BMap.Polyline(itemData, {
            strokeColor: "#ef4136",
            strokeWeight: 3,
            strokeOpacity: 1
          });
    ...
    // 画线
    if (servicePointList != null && servicePointList.length > 0) {
      var polyServiceLine = new window.BMap.Polyline(servicePointList, {
        strokeColor: "#ef4136",
        strokeWeight: 3,
        strokeOpacity: 1
